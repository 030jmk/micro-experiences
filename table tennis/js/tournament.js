...
(function(PS) {
  ...
  Tournament.prototype.reportResult = function(matchId, winnerSide, gameScores) {
    var m = this.matches[matchId];
    if (!m || m.winner !== null) return;
    m.winner = winnerSide;
    m.gameScores = gameScores || [];
  ...
  SingleElim.prototype._buildBracket = function() {
    ...
    for (var j = 0; j < size; j += 2) {
      ...
      if (m.players[0] !== null && m.players[1] === null) {
        m.winner = 0;
      } else if (m.players[0] === null && m.players[1] !== null) {
        m.winner = 1;
      }
      round1.push(m);
      this.matches[m.id] = m;
    ...
    for (var r = 1; r < numRounds; r++) {
      ...
      for (var k = 0; k < prevRound.length; k += 2) {
        ...
        prevRound[k].nextMatchId = m2.id;
        prevRound[k].nextSlot = 0;
        prevRound[k + 1].nextMatchId = m2.id;
        prevRound[k + 1].nextSlot = 1;
        round.push(m2);
  ...
  DoubleElim.prototype._buildBracket = function() {
    ...
    // Build losers bracket: alternates between "drop-down" rounds and "play" rounds
    ...
    for (var lr = 0; lr < numLRounds; lr++) {
      ...
      if (lr === 0) {
        ...
        var dropCount = Math.floor(prevLosers.length / 2);
        if (dropCount === 0) dropCount = prevLosers.length;
    ...
    // Reset match (if losers bracket winner wins GF)
    var rm = {
    ...
    for (var r = 0; r < rounds; r++) {
      ...
      for (var j = 0; j < total / 2; j++) {
        ...
        var m = {
          ...
          players: [p1, p2], winner: null, gameScores: [],
          bracket: 'round_robin'
        };
        roundMatches.push(m);
        this.matches[m.id] = m;
  ...
  RoundRobin.prototype._updateStandings = function() {
    ...
    Object.keys(this.matches).forEach(function(id) {
      ...
      map[w.id].wins++;
      map[l.id].losses++;
      (m.gameScores || []).forEach(function(gs) {
  ...
  GroupKnockout.prototype._buildKnockout = function() {
    ...
    // Cross-group pairing for knockout
    var koPlayers = [];
  ...
  function restoreTournament(json) {
    ...
    Object.keys(json.matches).forEach(function(mid) {
      if (t.matches[mid]) {
        ...
        t.matches[mid].gameScores = json.matches[mid].gameScores || [];
